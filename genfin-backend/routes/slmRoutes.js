...
router.post('/chat', async (req, res) => {
  try {
    ...
    const isProfileFilled = req.body.isProfileFilled || false;

    if (!message.trim()) {
      return res.json({ success: true,
        reply: 'Please ask a financial question.' });
    }

    // Detect if message needs personalized data
    const needsPersonalization = /my |mine|my plan|my investment|my score|my savings|my income|my debt|my profile|analyze me|my finances|my situation|personalized|for me|my goals|my risk|my emergency/i.test(message);

    // Detect if message needs GUI component
    const needsGUI = /investment plan|show plan|create plan|give me plan|scheme|government scheme|which scheme|suggest scheme|recommend|portfolio|allocation|show chart|visualize|graph|compare|breakdown/i.test(message);

    // If needs personalization but no profile
    if (needsPersonalization && !isProfileFilled) {
      return res.json({
        success: true,
        reply: `I'd love to give you personalized advice, 
but I need your financial profile first.
...
➡️ It takes less than 3 minutes!`,
        action: 'PROMPT_PROFILE',
        showButton: {
    ...
    // Get AI response
    const reply = await OllamaService.generateResponse(
      message, userId, userProfile
    );

    // Detect if response should include GUI
    let responseType = 'text';
    if (needsGUI && isProfileFilled) {
      responseType = 'gui';
    }
    ...
    res.json({
      ...
      reply,
      responseType,
      needsGUI: needsGUI && isProfileFilled,
      profile: isProfileFilled ? userProfile : null
    });
