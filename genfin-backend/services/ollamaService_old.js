  ...
  static getFallbackExplanation(data, analysisType) {
    ...
    switch (analysisType) {
      ...
      case 'chat': {
        ...
        if (!q || /^(hi|hello|hey|good\s+morning|good\s+evening|how\s+are\s+you|what's\s+up)/i.test(rawQuestion)) {
          return `Hello! I'm GenFin.ai, your financial assistant. I'm here to help you understand your finances and make smart money decisions. Feel free to ask me anything about budgeting, saving, investing, or financial planning. How can I help you today?`;
        }

        // 2. Personal questions - be friendly but guide to finance
        if (q.includes('name') || q.includes('who are you') || q.includes('what is your name')) {
          return `I'm GenFin.ai, your friendly financial assistant! I'm here to help you with all things money-related - from budgeting and saving to investing wisely. Think of me as your personal financial guide. What would you like to know about managing your finances better?`;
        }

        // 3. General questions - be conversational
        if (q.includes('what is') || q.includes('explain') || q.includes('tell me about')) {
          if (q.includes('sip')) {
            return `A Systematic Investment Plan (SIP) is a smart way to invest regularly! Instead of investing a lump sum, you invest small amounts monthly. It's like giving your savings a disciplined workout - builds wealth steadily and reduces market timing risks. Perfect for beginners! Want to know more about starting an SIP?`;
          }
          if (q.includes('mutual fund')) {
            return `Mutual funds are like pooling money with many investors to buy a variety of stocks/bonds. Professional managers handle the investments, so you don't have to pick individual stocks. Great for diversification! Based on your profile, I'd suggest starting with debt or hybrid funds. Curious about specific fund types?`;
          }
          if (q.includes('emergency fund')) {
            return `An emergency fund is your financial safety net - 3-6 months of expenses saved for unexpected situations like job loss or medical emergencies. It prevents you from breaking your investments during crises. Yours looks ${data.emergencyFundMonths >= 6 ? 'excellent!' : data.emergencyFundMonths >= 3 ? 'good but could be better' : 'like it needs attention'}. Want tips to build it faster?`;
          }
        }
