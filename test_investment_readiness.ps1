...
try {
    ...
    Write-Host "Investment readiness data:"
    if ($response.data.investmentReadiness) {
        Write-Host "Investment readiness: $($response.data.investmentReadiness | ConvertTo-Json -Depth 3)"
    }
} catch {
